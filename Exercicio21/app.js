...
const numbersImpar = randomNumbers.filter((item) => item % 2 === 1);
...
const numbersAbaixo50 = crazyNumbers.reduce((acc, item) => {
  if (item < 501) {
    acc += 1;
  }
...
const numbersQuadrado = numbers.map((item) => item ** 2);
...
const listaNome = cart.reduce((acc, produt) => {
  return `${acc}- ${produt.name}\n`;
}, "");
...
const filmesAntes2000 = tarantinoMovies.filter((item) => item.release < 2000);
